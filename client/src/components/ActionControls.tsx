    ...
    if (isMyTurn) {
      return (
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-800 bg-slate-900/70 p-4 text-sm text-slate-300">
          ...
          {!hasTargets && (
            <button className="rounded-lg bg-slate-700 px-3 py-1 text-xs font-semibold text-slate-100" onClick={() => onAction({ event: "pass" })} type="button">
              Continue
            </button>
          )}
        </div>
        ...
        {actionButtons.map((action) => {
          ...
          return (
            <button
              ...
              onClick={() => !isDisabled && onAction({ event: "perform_action", action: { type: action.type } })}
              type="button"
            >
              {action.label}
            </button>
